...
unsigned long local_time_t = 0;
unsigned long local_time_t_2 = 0;
...
void loop() {
  ...
  while(recievedData.count < MESSAGE_NUM){
      radio.openReadingPipe(1,address[LEADER]);  //read from the leader node
      radio.startListening();
      ...
      if (recievedData.checkSum == checkSum(recievedData.pos,recievedData.count,recievedData.id) && recievedData.id > 0){
          pos_arr[recievedData.count] = recievedData.pos;
          count_arr[recievedData.count] = recievedData.count;
          id_arr[recievedData.count] = recievedData.id;
      } else {
        recievedData.count = 0;
      }
  }   
...
void syncTime(unsigned long globalTime){
  if(globalTime != 0){
    local_time_t = globalTime;
  }
}
