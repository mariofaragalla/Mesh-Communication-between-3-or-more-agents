...
void loop() {
  ...
  if(rx_1st_time){
    ...
    for(int i = 0; i< trials - 1; i++){
      while(rx_count < MESSAGE_NUM*(i+1)){
          radio.openReadingPipe(1,address[LEADER]);  //read from the leader node
          radio.startListening();
          ...
          if (recievedData.checkSum == checkSum(recievedData.pos,recievedData.count,recievedData.id) && recievedData.id > 0){
              pos_arr[rx_count] = recievedData.pos;
              count_arr[rx_count] = recievedData.count;
              id_arr[rx_count] = recievedData.id;
              rx_count++;
          } else {
            recievedData.count = 0;
          }
  ...
  while(sentData.count < MESSAGE_NUM){
        ...
        pos_arr[sentData.count+20] = sentData.pos;
        count_arr[sentData.count+20] = sentData.count;
        id_arr[sentData.count+20] = sentData.id;
