...
for(int trials = 0; trials < 10; trials++){ 
  ...
  if(recieve_f){
    ...
    if(rx_1st_time){
       ...
       recievePacket(MESSAGE_NUM, trials, NODE3-1);
       
      rx_1st_time = false;
   } else{
    
      recievePacket(MESSAGE_NUM, trials, NODES_NUM-1);
   ...
   radio.flush_rx();
   sendPacket(MESSAGE_NUM, NODE3, trials);
...
void recievePacket(uint8_t numOfMessages , uint8_t numOftrials, uint8_t iterations){
      ...
      for(uint8_t i = 0; i< iterations; i++){
        ...
        for(uint8_t recieve_count = 0; recieve_count< numOfMessages; recieve_count++){
              ...
              if (recievedData.checkSum == checkSum(recievedData.pos,recievedData.count,recievedData.id) && recievedData.id > 0){
                  ...
                  counter_glb++;
//              if(recievedData.id == NODE3 && recieve_count == 0){
