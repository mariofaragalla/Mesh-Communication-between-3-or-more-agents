...
uint8_t my_id = 3;   // initial id of this node
...
//  radio.stopListening(); 
  ...
  delay(20);
  radio.stopListening();
  radio.openWritingPipe(node3Address);
}
...
bool isLeader(){  // this funciton is to decide if this node is the leader or not (decided by the server)
 ...
 delay(20);
 do{
    for(trials = 0; trials < 20 && !ackSent; trials++, ackSent = radio.write(&leaderData, sizeof(leaderData))) ;
    }while(!ackSent);
